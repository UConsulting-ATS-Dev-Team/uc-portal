...
**2026-09-03 -- Sixteenth addition: six more recognizable companies
(Anduril Industries, Rocket Lab, Zoox, SeatGeek, StubHub, Sweetgreen).**
Continues the big-name-first direction the Twelfth-through-Fifteenth
additions started (standing user direction: "I want to get big name
companies first. I've never heard of Marqeta."). Confirmed the live
starting count via direct Postgres query first (131 Greenhouse + 7 Lever
approved sources, matching the Fifteenth addition's own closing figure
exactly -- no drift since), then read every prior dated entry in this
Part (Sixth through Fifteenth additions) to compile the roster of 200+
companies already added or checked-and-rejected across fifteen prior
passes, so nothing here re-checked settled ground.

**~34 candidates checked**, spanning verticals the task brief flagged as
underexplored: airlines/hospitality, insurance, semiconductor/hardware,
biotech/pharma, defense/aerospace, real estate/proptech, food & beverage,
telecom, automotive/mobility, sports/media, fashion/retail, financial
data/market infrastructure. No usable board on any platform: Aurora
Innovation, Polestar, AMD, Arm Holdings, Garmin, Chipotle Mexican Grill,
TKO Group Holdings, Warby Parker, Chewy, Wayfair, Etsy, FactSet,
Morningstar, Intercontinental Exchange, Chubb, Alaska Airlines, JetBlue
Airways, Royal Caribbean Group, Vail Resorts -- extending the
now-well-established "legacy/traditional-industry companies run Workday
or nothing checkable" pattern cleanly into airlines, insurance, big-box
retail, and hardware, verticals not deeply tested before. Workday-hint-
only (not buildable via this codebase's public-API-only approach): Micron
Technology, Vertex Pharmaceuticals, Gilead Sciences, 23andMe, Nasdaq Inc,
CME Group, Live Nation Entertainment.

**One identity rejection worth recording in detail**: **"CoStar Group"**
-- Greenhouse slug "costar" resolves (8 postings, company_name
"Co–Star"), and at first glance looked plausible for the real
Washington DC-area commercial real-estate data giant (NASDAQ: CSGP). But
every sampled posting (Brand Designer, Haskell Developer, Junior
Operations Associate, Product Designer, all New York-based -- not
CoStar's real DC-area footprint) reads as a small NYC tech company, and a
direct page-source fetch of one job listing confirmed it self-references
"costarastrology.com" and "Astrology" repeatedly -- this is **Co-Star
Astrology**, a real, moderately well-known consumer astrology app, not
the real-estate data giant the search targeted. Same slug-collision
pattern already caught for "bcg"/Disney/Capital One/Aura/Wise/Current/
Founders Fund/Thrive Capital/Anchorage Capital Group/Charles Schwab/
MetLife/Blue Apron -- the user's own standing warning that short/famous
names are the highest-risk category for slug-squatting held up again.
Excluded (not independently pursued as "Co-Star Astrology" in its own
right either, since it wasn't the candidate this pass set out to verify).

**Six real hits**, each identity-verified the same way as every prior
addition -- `company_name` exact match, plus sampled application URLs,
office footprints, or self-referencing job content, never a slug guess
alone. A local pure-JS estimate against the exact `SENIOR_TITLE_PATTERN`/
`MANUAL_TRADE_TITLE_PATTERN`/`CLINICAL_CARE_TITLE_PATTERN` regexes in
`server/src/relevance.ts` was run against every live posting for all six
before writing the migrations (not assumed):

- **Anduril Industries** -- real, extremely high-profile defense-tech
  company (autonomous weapons systems/drones, founded by Palmer Luckey).
  Greenhouse slug "andurilindustries" (the plain "anduril" guess is not
  the real token -- same legal-name-style-variant technique as
  Wiz/Fanatics/Tenable), 2,207 postings -- the second-largest board this
  app has ever added, after SpaceX's 2,255. `company_name` "Anduril
  Industries" (exact). Office footprint (Costa Mesa CA -- real HQ, Fort
  Collins CO, Colorado Springs CO, Atlanta GA, Seattle WA) matches
  exactly. 1,081 of 2,207 sampled titles survive the relevance filter
  locally (2026/2027 Early Career Electrical/Mechanical/Software Engineer
  roles heavily represented, plus Financial Analyst/Product Manager
  roles); capped to 30 active.
- **Rocket Lab** -- real publicly-traded (NASDAQ: RKLB) space/launch
  company. Greenhouse slug "rocketlab" (obvious-guess hit), 452 postings,
  ...
  manufacturing-technician-heavy (Aerospace Production Technician, A\&P
  Mechanic, Assembly Integration \& Test Technician are common) but not
  Carvana-scale dominant -- only 46 of 452 titles (10%) trip the
  manual-trade denylist; 180 of 452 survive the relevance filter locally
  (Additive Manufacturing Engineer, Avionics Design Engineer, Flight
  Analysis Engineer among the white-collar engineering/business roles
  kept); capped to 30 active.
- **Zoox** (Lever) -- real autonomous-vehicle ("robotaxi") company, an
  Amazon subsidiary since 2020, high-profile within tech/mobility. Lever
  slug "zoox" (obvious-guess hit), 236 postings. No self-reported company
  name exists in Lever's response (same limitation documented for
  Wealthfront/Belvedere Trading/Tala/Spotify/Coupa) -- identity verified
  instead via every sampled posting located in Foster City, CA (Zoox's
  real HQ), zoox.com/careers's own page content directly referencing
  "Amazon" (Zoox's real parent), and strongly autonomous-vehicle-specific
  job content (Autonomy System Test Engineer, AV Safety Data Reporting
  Engineer, Chassis Controls Engineer). 102 of 236 sampled titles survive
  the relevance filter locally (only 1 of 236 trips the manual-trade
  denylist -- "Brakes" in an engineering title, "Chassis Controls
  Engineer, Brakes \& Steering," correctly caught by the same
  conservative word-boundary match that already excludes Carvana's actual
  brake-repair technicians); capped to 30 active.
- **SeatGeek** -- real event-ticketing marketplace, a widely recognized
  ...
  resolve directly to seatgeek.com/jobs/...?gh_jid=... (own domain), same
  own-domain-embed tier as Adyen/Klaviyo/Netskope/Wiz. NYC-based roles
  match SeatGeek's real HQ. Small board: 6 of 20 titles survive the
  relevance filter locally.
- **StubHub** -- real publicly-traded (NYSE: STUB) live-event ticket
  resale marketplace, a widely recognized consumer brand. Greenhouse slug
  "stubhubinc" (the bare "stubhub" guess does not resolve -- same
  legal-name-style-variant technique as Anduril/Wiz/Fanatics above), 17
  ...
  Manager among them).
- **Sweetgreen** -- real publicly-traded (NYSE: SG) fast-casual salad
  chain, a widely recognized consumer brand. Greenhouse slug "sweetgreen"
  ...
  the relevance filter locally, but honestly, the surviving composition
  leans heavily restaurant-operations (Assistant Restaurant Manager,
  Restaurant General Manager, Assistant/Head Coach -- Sweetgreen's
  internal term for shift lead) rather than HQ-corporate -- same
  already-documented "ambiguous manager/retail-corporate keep zone" this
  doc has recorded for Peloton/Lucid Motors/Glossier/StockX store-level
  titles (not manual-trade or clinical, so correctly not denylisted, but
  worth flagging plainly rather than implying an HQ-heavy board). A
  genuine minority of real corporate roles is also present (Manager
  Talent Acquisition, Quality Assurance Specialist, Recruiter, Tax
  Manager, Risks \& Claims Manager).
...
Added via two migrations onto the existing config-driven mechanisms --
`20260902160000_greenhouse_sixteenth_addition.sql` (the five Greenhouse
companies) and `20260902170000_lever_zoox.sql` (Zoox) -- no adapter code
changes needed, all six inherit Part 1's white-collar relevance filter
and Part 2's `MAX_ACTIVE_JOBS_PER_COMPANY = 30` cap automatically,
verified rather than assumed. Migrations pushed via `npx supabase db push
--linked` after confirming no timestamp collision against
`supabase/migrations`' actual tail immediately beforehand.

Verified live end-to-end via direct `curl` against both deployed HTTPS
endpoints (anon key). `fetch-greenhouse-companies`: first invocation
inserted correctly for all five (`skippedCompanyMismatch: 0` throughout);
SeatGeek/StubHub/Sweetgreen hit `deferred: 0` and confirmed idempotent
(`inserted: 0`, correct `refreshed` counts) on a second invocation.
Rocket Lab needed a second invocation to fully drain its
`MAX_NEW_JOBS_PER_RUN` backlog (`deferred: 30` → `0`), then confirmed
idempotent on a third. Anduril Industries' exceptional size (2,207
postings, second only to SpaceX) means its own backlog is still draining
after three invocations (`deferred: 931` → `781` → `631`) --
expected given even Anthropic's and SpaceX's own much smaller/larger
boards needed multiple runs each; not a bug, and Anduril is already
sitting at its correct capped 30 active regardless of how much backlog
remains. `fetch-lever-companies`: both invocations of Zoox returned
identical `inserted: 0, refreshed: 102` -- confirmed via a direct
Postgres timestamp check that these are genuine rows created within this
session's own invocation window (`created_at` ~1 minute before the
verification query, not pre-existing), so the field-naming quirk (all the
inserts landing under "refreshed" rather than "inserted" even on the
first real invocation) doesn't indicate stale or duplicate data -- both
runs agreeing on identical counts is itself the idempotency signal that
matters here.

Cross-checked directly against Postgres (`npx supabase db query
--linked`), not just fetch summaries: **Anduril Industries 30/450, Rocket
Lab Corporation 30/176, sweetgreen 30/48, Zoox 30/102** (all four hit the
30-job cap); **SeatGeek 6/6, StubHub 8/8** (under 30 because relevant
volume doesn't reach it, `capDeactivated: 0` for both, consistent with
every small-board addition in this doc's history). Pulled a 40-title
random sample across Anduril/Rocket Lab plus every active title for
SeatGeek/StubHub/sweetgreen/Zoox directly from Postgres: zero
manual-trade or clinical-care titles anywhere -- genuinely white-collar
engineering/finance/product/marketing/operations roles throughout
(Financial Analyst II, Full Stack Software Engineer, Propulsion Design
Engineer II, Client Success Manager, Associate Product Manager, Tax
Manager, among them), with Sweetgreen's restaurant-manager-heavy
composition noted honestly above rather than glossed over.

`npm run test:server`: **123/123 green**, unaffected (config-only
addition, no adapter/pipeline code touched this pass).

**Total company job-listing sources after this addition: 145** (136
Greenhouse + 8 Lever + 1 Deloitte RSS feed), confirmed via a direct count
...
internal/system sources, not company job-listing sources, and are
excluded from this count same as every prior addition's tally), up from
...
**2026-09-24 -- Eighteenth addition: two more Greenhouse companies, both
real consulting firms.** Direct autonomous-work-session ask, prioritized
per standing direction to check consulting firms first (UC is a
consulting club) before broadening to other verticals -- see
`data/industryBaseRates.js`'s own 2026-09-23 SOURCES note for the
identical priority applied to that feature.

Confirmed the live starting count via direct Postgres query first (169
total approved company sources -- 5 more than the Seventeenth addition's
closing 164, added in some intervening pass this doc doesn't have its own
entry for). Checked ~20 consulting-firm candidates across MBB-adjacent,
healthcare/life-sciences, brand/marketing, and restructuring/turnaround
consulting against both the Greenhouse and Lever public APIs.
...
Consulting, Huron Consulting Group, ICF, Booz Allen Hamilton, Simon-Kucher
& Partners, Cornerstone Research, Analysis Group, NERA Economic
Consulting, Bridgespan Group, Alvarez & Marsal, Berkeley Research Group,
Ankura Consulting, CrossCountry Consulting, GLG, Innosight, Eagle Hill
Consulting, Public Consulting Group -- extending the established
"elite/traditional consulting firms mostly run proprietary ATS, not
Greenhouse/Lever" pattern from prior additions into the consulting
vertical specifically. Accordion, AlixPartners, Baringa, Charles River
Associates, Point B, Deloitte, AlphaSights, and Third Bridge were already
live sources from earlier passes -- confirmed directly against the
`sources` table before spending any verification effort on them, not
re-checked or re-added (same discipline the Seventeenth addition's
Checkr/Rent the Runway near-duplicate taught).

One identity rejection, same slug-squatting risk class every prior
addition has flagged for a short/well-known-brand slug: "oliverwyman"
(Lever) resolves but is an unrelated small company, not the real Oliver
Wyman (Marsh McLennan) -- only 2 total postings ("Account Executive"/
"Software Engineer", San Francisco, generic SaaS-sales language, dated
2016), no consulting content or NYC/global-firm signal at all. Excluded.

One real-but-empty board: "stax"/"staxinc" (Lever) both resolve for what
appears to be the real Stax Inc (PE-focused strategy consulting) but
return zero active postings on either slug -- not worth adding an empty
source, same precedent as prior additions' empty-board exclusions.

**Two real hits**, each identity-verified the same way as every prior
...
- **ClearView Healthcare Partners** -- real healthcare/life-sciences
  strategy consulting firm (pharma/biotech commercial strategy, market
  access, launch planning). Greenhouse slug
  "clearviewhealthcarepartners", 31 postings, `company_name` exact.
  Newton, MA location matches the real firm's actual HQ; "Consulting
  Project Team Lead -- Pricing & Market Access", "Life Sciences Strategy
  Consultant", "Consulting Project Team Lead -- Corporate Strategy"
  titles are exactly the real firm's known practice areas. 31 of 31
  sampled titles survive the relevance filter locally (100%
  white-collar, no manual-trade/clinical-care hits); will be capped to
  30 active.
- **Prophet** -- real brand/marketing strategy consulting firm (San
  Francisco HQ). Greenhouse slug "prophet", 21 postings, `company_name`
  exact. Offices sampled (Austin, Chicago, NYC, Atlanta, Zurich, Munich,
  Berlin, Hong Kong, Shanghai, London) match the real firm's actual
  global footprint; "2027 Summer Associate -- Austin" directly signals
  real undergraduate campus recruiting, and "Engagement Manager"/
  "Associate" titles match standard consulting-firm level naming. 21 of
  21 sampled titles survive the relevance filter locally.

Neither matches a `NAMED_COMPANY_RATES` entry in
`data/industryBaseRates.js` (MBB/bulge-bracket IB/elite quant/elite big
tech) -- both fall through to that module's broader industry-tier
fallback via their real "Management consulting"-adjacent classification,
same as most of this app's real sources today.
...
Deliberately did not force a manual full-company fetch to verify
ingestion synchronously -- a direct invocation of
`fetch-greenhouse-companies` hit a real `WORKER_RESOURCE_LIMIT` error
processing all 171 configured companies in one synchronous HTTP call
(distinct from how `pg_cron`'s own `net.http_post` triggers it --
fire-and-forget, not held open waiting for the full response the way a
synchronous `curl` call is). Checked the real scheduled-run history
instead (`cron.job_run_details` for `fetch-greenhouse-companies-daily`):
succeeded on every one of the last 5 daily runs (Sept 19-23), confirming
the pipeline itself is genuinely healthy: the two new companies will be
picked up automatically on the next scheduled run rather than needing a
forced manual one, same "don't force an extra live run, trust the daily
schedule" precedent `check-job-links`'s own cadence-fix entry already
established.

**Total company job-listing sources after this addition: 166** (155
Greenhouse + 10 Lever + 1 Deloitte RSS feed), confirmed via a direct
count against the live `sources` table grouped by `config->>'platform'`
(the same 4 internal/system sources excluded from every prior addition's
